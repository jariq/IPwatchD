...
void ipwd_analyse(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
  ...
  if (debug_flag) {
    snprintf(msgbuf, IPWD_MSG_BUFSIZ, "***\nRECEIVED ARP PACKET: S:%s|%s D:%s|%s\n", rcv_sip, rcv_smac, rcv_dip, rcv_dmac);
  ...
  for (i = 0; i < devices.devnum; i++) {
    ...
    if (ipwd_devinfo(devices.dev[i].device, devices.dev[i].ip, devices.dev[i].mac) == IPWD_RV_ERROR) {
      snprintf(msgbuf, IPWD_MSG_BUFSIZ, "Unable to get IP and MAC address of %s\n", devices.dev[i].device);
      ipwd_message(msgbuf, IPWD_MSG_ERROR);
      return;
    }
    ...
    if ((strcmp(rcv_sip, devices.dev[i].ip) == 0) && (strcmp(rcv_smac, devices.dev[i].mac) != 0)) {
      ...
      if (devices.dev[i].mode == IPWD_MODE_ACTIVE) {
        ...
        snprintf(msgbuf, IPWD_MSG_BUFSIZ, "MAC address %s causes IP conflict with address %s set on interface %s - reply sent.\n", rcv_smac, devices.dev[i].ip, devices.dev[i].device);
        ...
        snprintf(msgbuf, IPWD_MSG_BUFSIZ, "MAC address %s causes IP conflict with address %s set on interface %s - reply NOT sent.\n", rcv_smac, devices.dev[i].ip, devices.dev[i].device);
        ipwd_message(msgbuf, IPWD_MSG_ALERT);
      ...
      if (debug_flag) {
        snprintf(msgbuf, IPWD_MSG_BUFSIZ, "Not conflicting with: %s : %s|%s\n", devices.dev[i].device, devices.dev[i].ip, devices.dev[i].mac);
        ipwd_message(msgbuf, IPWD_MSG_INFO);
      }
